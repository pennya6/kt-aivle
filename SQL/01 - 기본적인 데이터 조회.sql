...
select emp_id,emp_name,hire_date from employee where gender="M";

-- Q) 2016년 6월 1일 입사자의 이름, 부서코드, 성별, 이메일 정보 조회
select emp_name,dept_id,gender,email from employee where hire_date='2016-06-01';
...
select * from employee where dept_id='SYS' and year(hire_date)>=2018;

-- Q) SYS 부서 직원 중에서 급여를 6,000 이상 받는 직원 정보 조회
select * from employee where salary >=6000 and dept_id="SYS";
...
select * from employee where dept_id in('SYS','MKT') and year(hire_date)=2014;

-- Q) 2015년도에 입사한 연봉이 6,000 이상인 근무중인 직원 정보 조회
select * from employee where year(hire_date)=2015 and salary>=6000;
...
select emp_name, ifnull(eng_name," ") as eng_name from employee where eng_name is null ;
...
select emp_name, 
		case when salary <5000 then '하' 
		 when salary <7000 and salary>=5000 then '중' 
         when salary>=7000 then '상' 
         else "기타" end as salary
from employee;
...
-- Q) 급여 크기를 상, 중, 하로 구분
select emp_name, if (salary >=7000,'상','중') as salary
from employee;
