...
CREATE TABLE vacation (
	...
	duration INT AS (DATEDIFF(end_date, begin_date) + 1), -- 날짜 차이 계산해서 +1
	CHECK (end_date >= begin_date), -- 규칙 선언
...
CREATE TABLE friend (
    ...
    friend_name char(4) NOT NULL,
    friend_tel char(10) NOT NULL,
	friend_address char(20) NOT NULL,
    friend_age int not null,
    PRIMARY KEY (friendfriendfriend_id)
);
...
alter table orders add amount int not null;
 
-- 확인
DESCRIBE orders;
 
-- Q7) orders 테이블 이름을 order_header로 변경하세요.
alter table orders rename to order_header;
